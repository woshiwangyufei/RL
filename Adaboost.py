...
# 计算相关性矩阵
data_Voc = pd.concat([X, y], axis=1)
Voc_corr = data_Voc.corr().round(3)

# 绘制相关性热图
from matplotlib.colors import LinearSegmentedColormap

# 定义一个自定义颜色映射，从蓝色到红色
colors = ["#0000ff", "#ffffff", "#ff0000"]  # 从蓝色到白色再到红色
custom_cmap = LinearSegmentedColormap.from_list("blue_red", colors)

# # 绘制相关性热图
# plt.figure(figsize=(40, 40))
# font_path = "C:\\Users\\wjf18\\Desktop\\毕业论文\\工作一容量剩余容量\\剩余容量\\专业\\timesbd.ttf"
# font_prop = fm.FontProperties(fname=font_path, size=20, weight='bold')
# matplotlib.rcParams['font.family'] = font_prop.get_name()
# matplotlib.rcParams['font.size'] = font_prop.get_size()
#
# ax = sns.heatmap(Voc_corr, annot=True, vmin=-1, vmax=1, fmt='.3f', square=True, cmap=custom_cmap, annot_kws={'weight': 'bold', "fontsize": 20})
# plt.show()
...
param_grid = {
    ...
    'estimator__min_samples_split': [2], #减小分裂限制 可以解决欠拟合的现象
    'estimator__min_samples_leaf': [3], #减小叶节点 缓解过拟合的现象
